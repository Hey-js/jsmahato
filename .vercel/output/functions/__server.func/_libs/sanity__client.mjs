...
function defineHttpRequest(envMiddleware, config = {}) {
  return k([
    ...
    _(),
    httpError,
    N({ implementation: cjsExports.Observable })
  ]);
}
function shouldRetry(err, attempt, options) {
  ...
  return (isSafe || isQuery2) && isRetriableResponse ? true : z.shouldRetry(err, attempt, options);
}
...
const pick = (obj, props) => props.reduce((selection, prop) => (typeof obj[prop] > "u" || (selection[prop] = obj[prop]), selection), {}), eventSourcePolyfill = cjsExports.defer(() => import("./sanity__eventsource.mjs").then(function(n) {
  return n.n;
})).pipe(
...
class LiveClient {
  ...
  events({
    ...
    const events = connectEventSource(() => (
      ...
      cjsExports.catchError((err) => err instanceof CorsOriginError ? cjsExports.throwError(() => err) : checkCors.pipe(
        cjsExports.mergeMap(() => {
          throw err;
        })
      )),
...
class DatasetsClient {
   ...
   */
  getEmbeddingsSettings(name2) {
    return resourceGuard("dataset", this.#client.config()), dataset(name2), cjsExports.lastValueFrom(
...
function _embeddingsSettingsUri(client, name2) {
  const config = client.config();
  return config.useProjectHostname === false ? `/projects/${config.projectId}/datasets/${name2}/settings/embeddings` : `/datasets/${name2}/settings/embeddings`;
}
...
class ProjectsClient {
  ...
  list(options) {
    const query = {}, uri = "/projects";
    return options?.includeMembers === false && (query.includeMembers = "false"), options?.includeFeatures === false && (query.includeFeatures = "false"), options?.organizationId && (query.organizationId = options.organizationId), options?.onlyExplicitMembership && (query.onlyExplicitMembership = "true"), cjsExports.lastValueFrom(
...
const middleware = [
  y$1({ verbose: true, namespace: "sanity:client" }),
  g({ "User-Agent": `${name} ${version}` }),
...
function walkMap(value, mappingFn, path = []) {
  ...
  if (isRecord(value)) {
    ...
    return Object.fromEntries(
      Object.entries(value).map(([k2, v]) => [k2, walkMap(v, mappingFn, path.concat(k2))])
    );
  }
